...
// Inklusive verbesserter "Fuzzy" Titel-Erkennung fuer abkuerzende Uploader.
//===============
...
function verifyTitleMatch(filename, searchTitles) {
    ...
    for (const title of searchTitles) {
        ...
        // 1. Exakter Substring Match (mit Leerzeichen)
        if (stripped.includes(cleanTitle)) return true;

        // 2. Normale Wort-Grenzen Suche
        const escapedTitle = cleanTitle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const regex = new RegExp(`(?:^|\\s)${escapedTitle}(?:\\s|$)`, "i");
        if (regex.test(stripped)) return true;

        // 3. Fallback: Ignoriere Leerzeichen komplett
        const cleanNoSpaces = cleanTitle.replace(/\s+/g, "");
        if (cleanNoSpaces.length > 4 && strippedNoSpaces.includes(cleanNoSpaces)) return true;
        
        // 4. "Fuzzy" Match fuer abgekuerzte Uploader-Titel (z.B. "Android wa Keiken Ninzuu" statt voller Name)
        const words = cleanTitle.split(" ");
        if (words.length >= 3) {
            // Pruefe, ob zumindest die ersten 3-4 markanten Woerter im Torrent-Namen auftauchen
            const partialMatchLength = Math.min(4, words.length - 1); // Nimmt die ersten max 4 Woerter, laesst aber mind. eins weg
            const partialTitle = words.slice(0, partialMatchLength).join(" ");
            
            // Vermeide Matches auf extrem kurze Fragmente (z.B. "The 100")
            if (partialTitle.length > 8) {
                 if (stripped.includes(partialTitle)) return true;
                 const partialNoSpaces = partialTitle.replace(/\s+/g, "");
                 if (strippedNoSpaces.includes(partialNoSpaces)) return true;
            }
        }
    }
    return false;
...
function extractEpisodeNumber(filename, expectedSeason = 1) {
    ...
    // Hentai-Konventionen: Text-Zahlen & Römische Ziffern übersetzen (z.B. Part Two -> Part 2, Vol. II -> Vol 2)
...
function extractLooseEpisode(filename) {
    ...
    // Auch hier die Übersetzung für Standalone-Tags
    const romanMap = { "i": 1, "ii": 2, "iii": 3, "iv": 4, "v": 5, "vi": 6, "vii": 7, "viii": 8, "ix": 9, "x": 10 };
