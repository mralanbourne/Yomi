//===============
// AMATSU & YOMI PARSING ENGINE (UNIFIED, FIXED FOR BATCHES & SMART SIZE FILTER)
//===============
...
function selectBestVideoFile(files, requestedEp, expectedSeason = 1, isMovie = false) {
    ...
    // ==========================================
    // 🛡️ SMART FILE SIZE FILTER (Anti False-Positive)
    // ==========================================
    if (!isMovie) {
        videoFiles = videoFiles.filter(f => {
            const size = f.size !== undefined ? f.size : (f.bytes || 0);
            if (size === 0) return true; // Unbekannte Größe vorsichtshalber behalten
            
            // 30 MB Minimum (filtert Teaser, NCOP, NCED, Menu-Dateien aus Batches)
            const MIN_SIZE = 30 * 1024 * 1024; 
            
            // 4.5 GB Maximum (Schützt BDRip 4K FLAC Releases, filtert aber 10GB Film-Dateien oder zusammenhängende Staffeln)
    ...
    if (videoFiles.length === 0) return null; // Wenn alle Videofiles im Filter hängen blieben, gab es keine valide Episode
