...
function extractEpisodeNumber(filename, expectedSeason = 1) {
    ...
    const explicitRegex = /(?:ep(?:isode)?\.?\s*|ova\s*|oad\s*|special\s*|round\s*|act\s*|chapter\s*|part\s*|vol(?:ume)?\.?\s*|第\s*|#\s*|s(\d+)\s*e|season\s*(\d+)\s*ep(?:isode)?\s*)0*(\d+)(?:\s*(?:巻|話))?(?:\D|$)/i;
...
function isEpisodeMatch(name, requestedEp, expectedSeason = 1) {
    ...
    if (explicitMatch) {
        ...
        return parseInt(explicitMatch[3], 10) === epNum;
    }

    const dashMatch = clean.match(/(?:^|\s)\-\s+0*(\d+)(?:\D|$)/i);
    if (dashMatch) {
        return parseInt(dashMatch[1], 10) === epNum;
    }
    
    const bracketMatch = clean.match(/\[0*(\d+)\]|\(0*(\d+)\)/i);
    if (bracketMatch) {
        return parseInt(bracketMatch[1] || bracketMatch[2], 10) === epNum;
    }
    ...
    if (epNum === 1 && (extractedEp === null || extractedEp === 0) && !batch) {
