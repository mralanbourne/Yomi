//===============
// AMATSU & YOMI PARSING ENGINE (UNIFIED, FIXED FOR BATCHES, SMART SIZE & SLOP PREVENTION)
//===============
...
function extractLooseEpisode(filename) {
    ...
    // STRICTER REGEX: Prevents capturing "20" from "20th" or mid-word strings.
...
function isSeasonBatch(filename, expectedSeason) {
    ...
    // REMOVED SLOPPY KEYWORDS (tv, series, eps, episodes, seasons, ova)
...
function selectBestVideoFile(files, requestedEp, expectedSeason = 1, isMovie = false) {
    ...
    // ==========================================
    // 🛑 SMART FALLBACK (ANTI-SLOP SHIELD)
    // ==========================================
    // Wenn der Torrent nur eine einzige Videodatei hat, servieren wir sie, da der Titel validiert wurde.
    if (videoFiles.length === 1) {
        return videoFiles[0];
    }

    // Wenn der Torrent mehrere Dateien hat (ein Batch ist), wir die gewünschte Episode aber nicht finden können, 
    // dann verweigern wir die Ausgabe! (Verhindert, dass Ep 1020 als Ep 20 serviert wird)
    return null;
}
