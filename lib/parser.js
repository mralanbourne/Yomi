//===============
// AMATSU & YOMI PARSING ENGINE (CJK & BATCH OPTIMIZED)
// Unterstützt japanische, chinesische und koreanische Titelformate sowie komplexe Batches.
//===============
...
function normalizeTitle(text) {
    ...
    let normalized = text.toLowerCase()
        .replace(/[\u200B-\u200D\uFEFF]/g, "")
        .replace(/[\u3000]/g, " "); // CJK Space to normal space

    // Mapping von CJK Zahlen zu Arabischen Zahlen
    const cjkMap = { '一': '1', '二': '2', '三': '3', '四': '4', '五': '5', '六': '6', '七': '7', '八': '8', '九': '9', '十': '10' };
    for (const [cjk, num] of Object.entries(cjkMap)) {
        normalized = normalized.replace(new RegExp(cjk, 'g'), num);
    }
...
function verifyTitleMatch(filename, searchTitles) {
    ...
    for (const title of searchTitles) {
        ...
        // 1. Exakter Boundary Match
        const escapedTitle = cleanTitle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const regex = new RegExp(`(?:^|\\s)${escapedTitle}(?:\\s|$)`, "i");
        if (regex.test(stripped)) return true;

        // 2. No-Space Match (Fuer zusammengeklebte CJK Titel)
        const cleanNoSpaces = cleanTitle.replace(/\s+/g, "");
        if (cleanNoSpaces.length >= 6 && strippedNoSpaces.includes(cleanNoSpaces)) {
            return true;
        }

        // 3. Smart Sequence / Prefix Match
        ...
        if (queryWords.length >= 2) {
            ...
            // 60% Match Ratio oder die ersten zwei Worte passen
            ...
            // Abkürzungs-Check: Wenn Torrent-Wörter alle in der Query sind
...
function extractEpisodeNumber(filename, expectedSeason = 1) {
    ...
    // Text-Zahlen & Römische Ziffern & CJK Zahlen
    ...
    clean = clean.replace(/\b(?:part|vol(?:ume)?|chapter|episode|ep|act|round|話|话|集|회|편)\.?\s*(one|two|three|four|five|six|seven|eight|nine|ten|i|ii|iii|iv|v|vi|vii|viii|ix|x)\b/ig, (m, p1) => {
        return m.replace(p1, wordMap[p1.toLowerCase()]);
    });

    // CJK Season/Episode Tags entfernen um Nummern zu isolieren
...
function getBatchRange(filename) {
    let clean = sanitizeFilename(filename);
    // Erkennt 01-12, 01~12, 01..12, 01 to 12
    ...
    // Erkennt CJK "Alle X Folgen" -> 全12話
...
function isSeasonBatch(filename, expectedSeason) {
    ...
    const hasBatchWord = /\b(batch|complete|collection|boxset|box-set|box\b|bd-box|dvd-box|all episodes|all eps|disc)\b|全集|완결|전편|全話/i.test(clean);
    if (hasBatchWord || getBatchRange(filename)) return true;
    
    // Wenn keine Episode gefunden wurde, aber der Titel passt, ist es wahrscheinlich ein Batch oder Movie
...
function isEpisodeMatch(name, requestedEp, expectedSeason = 1) {
    ...
    // Sequel-Schutz: Wenn Staffel > 1, muss ein Staffel-Tag vorhanden sein
...
function selectBestVideoFile(files, requestedEp, expectedSeason = 1, isMovie = false) {
    ...
    // Größen-Filter für Serien-Episoden (verhindert Slop-Dateien wie Trailer)
    if (!isMovie) {
        videoFiles = videoFiles.filter(f => {
            const s = f.size || f.bytes || 0;
            return s === 0 || (s >= 30 * 1024 * 1024 && s <= 6 * 1024 * 1024 * 1024);
        });
    }
