...
function verifyTitleMatch(filename, searchTitles) {
    ...
    let stripped = filename.replace(/\[.*?\]/g, " ").replace(/\(.*?\)/g, " ").toLowerCase();
    stripped = stripped.replace(/[_\.\+]/g, " ").replace(/\s+/g, " ").trim();

    for (const title of searchTitles) {
        if (!title) continue;
        const cleanTitle = title.toLowerCase().replace(/[_\.\+]/g, " ").replace(/\s+/g, " ").trim();
        ...
        if (/[^\x00-\x7F]/.test(cleanTitle) && stripped.includes(cleanTitle)) {
            return true;
        }
    }
    return false;
