...
//===============
// STRICT TITLE VERIFICATION (Amatsu/Nyaa)
//===============
...
//===============
// FUZZY TITLE VERIFICATION (Yomi/Sukebei)
//===============
function isCJK(str) {
...
function fuzzyTitleMatch(filename, searchTitles) {
    if (!searchTitles || searchTitles.length === 0) return true;
    ...
    return cleanSearchTerms.some(term => normalizedFilename.includes(term));
}
...
function isSeasonBatch(filename, expectedSeason) {
    ...
    if (hasSeasonTag || hasBatchWord) {
        ...
        if (explicitEp === null || explicitEp === -1) return true;
    }
    
    if (extractEpisodeNumber(filename, expectedSeason) === null && extractLooseEpisode(filename) === null && getBatchRange(filename) === null) {
        return true;
    }
    return false;
}
