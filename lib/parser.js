...
function verifyTitleMatch(filename, searchTitles) {
    ...
    for (const title of searchTitles) {
        ...
        // 1. Exakter Boundary Match (Bester Schutz gegen False Positives)
        const escapedTitle = cleanTitle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const regex = new RegExp(`(?:^|\\s)${escapedTitle}(?:\\s|$)`, "i");
        if (regex.test(stripped)) return true;

        // 2. Ohne-Leerzeichen Match (Fuer Uploader die Woerter zusammenkleben: "NatsutoHako")
        const cleanNoSpaces = cleanTitle.replace(/\s+/g, "");
        if (cleanNoSpaces.length >= 6 && strippedNoSpaces.includes(cleanNoSpaces)) {
            // Verhindert False Positives bei sehr kurzen Worten in riesigen Dateinamen
            if (cleanNoSpaces.length < 10 && strippedNoSpaces.length > cleanNoSpaces.length * 3) continue;
            return true;
        }

        // 3. Robuster Fallback fuer Uploader-Abkuerzungen (Zieht exakt die ersten 3-4 Woerter heran)
        const words = cleanTitle.split(/\s+/);
        if (words.length >= 3) {
            const requiredWords = words.slice(0, Math.min(4, words.length)).join(" ");
            if (requiredWords.length >= 8) {
                const reqEscaped = requiredWords.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                const reqRegex = new RegExp(`(?:^|\\s)${reqEscaped}(?:\\s|$)`, "i");
                if (reqRegex.test(stripped)) return true;
            }
        }
    }
    return false;
