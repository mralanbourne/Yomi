...
function verifyTitleMatch(filename, searchTitles) {
    ...
    for (const title of searchTitles) {
        ...
        // Normale Wort-Grenzen Suche
        const escapedTitle = cleanTitle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const regex = new RegExp(`(?:^|\\s)${escapedTitle}(?:\\s|$)`, "i");

        if (regex.test(stripped)) return true;

        // Fallback: Ignoriere Leerzeichen komplett (Hilft bei "NatsutoHako" statt "Natsu to Hako")
        // Wird nur bei Titeln > 4 Zeichen angewendet, um False-Positives bei sehr kurzen Woertern zu vermeiden.
