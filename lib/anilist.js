...
async function searchAdultAnime(query) {
    ...
    const graphqlQuery = `query ($search: String) { Page(page: 1, perPage: 50) { media(search: $search, type: ANIME, isAdult: true) { id title { romaji english } coverImage { extraLarge } bannerImage description format episodes averageScore status seasonYear startDate { year month day } } } }`;
    return _fetchAniList(graphqlQuery, { search: query });
}

async function getTrendingAdultAnime() {
    const graphqlQuery = `query ($sort: [MediaSort]) { Page(page: 1, perPage: 30) { media(type: ANIME, isAdult: true, sort: $sort) { id title { romaji english } coverImage { extraLarge } bannerImage description format episodes averageScore status seasonYear startDate { year month day } } } }`;
...
async function getLatestAdultAnime() {
    const graphqlQuery = `query ($sort: [MediaSort], $status: [MediaStatus]) { Page(page: 1, perPage: 30) { media(type: ANIME, isAdult: true, sort: $sort, status_in: $status) { id title { romaji english } coverImage { extraLarge } bannerImage description format episodes averageScore status seasonYear startDate { year month day } } } }`;
...
async function getAnimeMeta(anilistId, retries = 3) {
    ...
    const graphqlQuery = `query ($id: Int) { Media(id: $id, type: ANIME) { id idMal title { romaji english } synonyms coverImage { extraLarge } bannerImage description format episodes nextAiringEpisode { episode airingAt } streamingEpisodes { title thumbnail } averageScore status seasonYear startDate { year month day } } }`;
