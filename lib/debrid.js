...
/**
 * Scans Real-Debrid cache and returns all file names and IDs for matching.
 * Used for both stream resolution and dynamic meta generation.
 */
    ...
    try {
        ...
        Object.keys(res.data).forEach(hash => {
            ...
            if (availability && availability.rd && availability.rd.length > 0) {
                let allFiles = [];
                // Collect files from the first available variant
...
/**
 * Scans Torbox cache including full file listing.
 */
