...
// Mit globaler AsyncQueue zur strikten API-Ratelimit-Praevention.
//===============
...
// Strikte Limits: Nur 1 Call zeitgleich pro Provider, um 403/429 Banns zu vermeiden.
...
async function checkRD(hashes, apiKey) {
    ...
    const performFetch = async () => {
        try {
            ...
            if (status === 403 || status === 401) console.error(`[DEBRID-CRITICAL] ❌ 403/401 FORBIDDEN: Dein API-Key (${safeKey.substring(0,4)}...) ist ungültig, abgelaufen, oder StremThru blockiert dich!`);
            else if (status === 429) console.error(`[DEBRID-CRITICAL] ⏳ 429 RATE LIMIT: Du sendest zu viele Anfragen an StremThru.`);

            let errorTtl = (status === 401 || status === 403 || status === 429) ? 3600000 : 30000;
