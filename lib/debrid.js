...
async function getActiveTorbox(apiKey) {
    try {
        // FIX: Der korrekte Endpunkt ist /mylist, nicht /me
        const res = await axios.get('https://api.torbox.app/v1/api/torrents/mylist', { headers: { Authorization: `Bearer ${apiKey}` } });
        const active = {};
