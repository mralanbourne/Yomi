...
// Hochrobuste Live-Version mit nativer FlareSolverr Integration.
// Nutzt Headless-Proxying um Cloudflare JS-Challenges (HTTP 429/403) proaktiv zu umgehen.
//===============
...
async function fetchWithDomainQueue(url, domain, delayMs) {
    ...
    // 1. Prioritaet: FlareSolverr Bypass (falls konfiguriert)
    if (FLARESOLVERR_URL) {
        try {
            const payload = {
                cmd: "request.get",
            ...
            // Extrahieren des HTML/XML-Outputs aus dem Headless-Browser
            if (response.data && response.data.solution && response.data.solution.response) {
                return { data: response.data.solution.response };
            }
        } catch (error) {
            console.log(`[FLARESOLVERR ERROR] ❌ Fehler im Proxy-Tunnel (${error.message}). Falle auf nativen Request zurueck.`);
        }
    }

    // 2. Fallback: Normaler Axios-Call
