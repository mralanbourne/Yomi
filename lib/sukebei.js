...
// Strict 9-Second Provider Timeouts & Elevated Queue Concurrency.
//===============
...
// Concurrency auf 10 erhöht, um Warteschlangen-Staus (Blockaden) zu verhindern
const scrapeQueue = new AsyncQueue(10);
...
const activeSearches = new Map(); // Speichert aktuell laufende Suchen zur Deduplizierung
...
//===============
// TIMEOUT WRAPPER
// Garantiert, dass kein einzelner Scraper Stremio in ein Timeout zwingt.
//===============
const withTimeout = (promise, ms, providerName) => {
    return Promise.race([
        promise,
        new Promise((_, reject) => setTimeout(() => reject(new Error(`[TIMEOUT] ${providerName} brauchte laenger als ${ms}ms.`)), ms))
    ]);
};
...
function parseRSS(xml, sourceName) {
    ...
    while ((match = itemRegex.exec(xml)) !== null) {
        ...
        const seedMatchTor = itemMatch = itemXml.match(/<torrent:seeds>(\d+)<\/torrent:seeds>/);
...
//===============
// BYPASS LOGIK MIT QUEUE & VALIDIERUNG
//===============
async function fetchWithBypass(url, domainName) {
    return scrapeQueue.enqueue(async () => {
        if (FLARESOLVERR_URL) {
            try {
                // Timeouts drastisch reduziert, damit FlareSolverr nicht die ganze Node.js Queue blockiert.
                // 9 Sekunden maxTimeout fuer FlareSolverr.
                const res = await axios.post(FLARESOLVERR_URL, { cmd: "request.get", url: url, maxTimeout: 9000 }, { timeout: 12000 });
                if (res.data && res.data.solution && res.data.solution.response) {
                    ...
                    if (!html.includes("<rss") && !html.includes("<channel")) {
                        throw new Error("FlareSolverr lieferte ungueltiges XML/HTML.");
                    }
                    console.log(`[BYPASS] 🛡️  FlareSolverr hat Cloudflare für ${domainName} erfolgreich umgangen.`);
                    return { data: html };
                }
            } catch (e) { 
                console.log(`[BYPASS] ⚠️  FlareSolverr Fehler für ${domainName}. Falle auf nativen Request zurück.`); 
            }
        }
        
        const nativeRes = await axios.get(url, { timeout: 8000, headers: { "User-Agent": "Mozilla/5.0" } });
        if (nativeRes.data && typeof nativeRes.data === "string") {
            if (!nativeRes.data.includes("<rss") && !nativeRes.data.includes("<channel")) {
                throw new Error("Nativer Request wurde von Cloudflare blockiert.");
            }
        }
        return nativeRes;
    });
...
async function searchSukebeiForHentai(queryOriginal) {
    ...
    // 2. IN-FLIGHT DEDUPLIZIERUNG
    // Verhindert Stremio's Thundering Herd Problem, wenn Kataloge laden.
    if (activeSearches.has(queryKey)) {
        console.log(`[HUB] ⏳ Warte auf bereits laufende Suche für: "${queryOriginal}"...`);
    ...
    // Die gesamte Suchlogik als Promise verpacken, um sie in activeSearches zu speichern
    const searchExecutionPromise = (async () => {
            ...
            return scrapeQueue.enqueue(async () => {
                try {
                    ...
                    for (const item of topResults) {
                        ...
                        await new Promise(r => setTimeout(r, 300)); // Sleep drastisch reduziert
                    }
                    if (finalResults.length > 0) console.log(`[RUTRACKER] ✅ RuTracker lieferte ${finalResults.length} verifizierte Hashes.`);
                    return finalResults;
                } catch (error) { return []; }
            });
        };

        // Jeder Provider bekommt hart max 9.5 Sekunden, Stremio wartet nicht ewig!
        const MAX_PROVIDER_MS = 9500;
        const tasks = [ 
        ...
        const allResults = promiseResults.map(r => {
            ...
            // Wenn der Task ins Timeout gelaufen ist, loggen wir das stumm und gehen weiter.
            return [];
        }).flat();
        ...
        console.log(`[HUB] 🏆 Gesamt nach Deduplizierung: ${finalArr.length} Torrents für "${queryOriginal}".`);
        
        if (finalArr.length > 0) searchCache.set(queryKey, { data: finalArr, expiresAt: Date.now() + CACHE_TTL_MS });
        return finalArr;
    })();

    // Den laufenden Prozess in die Map legen
    ...
    try {
        ...
        // Am Ende IMMER aufräumen, egal ob Erfolg oder Error
