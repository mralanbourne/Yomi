...
// Mit erweitertem Deep-Logging für Provider-Transparenz.
//===============
...
async function searchSukebeiForHentai(queryOriginal) {
    ...
    const fetchAniRena = async () => {
        if (Date.now() < anirenaCooldownEnd) {
            console.log(`[ANIRENA] ⏳ Cooldown aktiv. Überspringe Suche.`);
            return [];
        }
        ...
        try {
            ...
            if (missingHashResults.length > 0) {
                ...
                for (let i = 0; i < missingHashResults.length; i += 2) {
                    if (rateLimitHit) break; 
                    const batch = missingHashResults.slice(i, i + 2);
                    await Promise.all(batch.map(async (item) => {
                        try {
                            ...
                            if (e.response && e.response.status === 429) {
                                ...
                                console.log(`[ANIRENA] 🚨 Torrent-Download Rate Limit (429)! Cooldown: ${retryAfter}s`);
                            }
                        }
                    }));
                    if (!rateLimitHit && i + 2 < missingHashResults.length) await new Promise(res => setTimeout(res, 500)); 
                }
