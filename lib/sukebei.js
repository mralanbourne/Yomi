...
// Geschuetzt durch AsyncQueue und strikte RSS-Validierung gegen Cloudflare-Illusionen.
//===============
...
// Diese Funktion garantiert, dass Cloudflare-Blockaden nicht als leere RSS-Feeds durchrutschen.
//===============
async function fetchWithBypass(url, domainName) {
    return scrapeQueue.enqueue(async () => {
        ...
        if (FLARESOLVERR_URL) {
            try {
                // Timeout auf 25s erhoeht, da Cloudflare Challenges extrem rechenintensiv sind
                const res = await axios.post(FLARESOLVERR_URL, { cmd: "request.get", url: url, maxTimeout: 25000 }, { timeout: 30000 });
                if (res.data && res.data.solution && res.data.solution.response) {
                    const html = res.data.solution.response;
                    
                    // VALIDIERUNG: Cloudflare gibt bei der Challenge oft HTTP 200 zurueck.
                    // Wenn kein <rss oder <channel Tag existiert, ist es kein valider Feed.
                    if (!html.includes("<rss") && !html.includes("<channel")) {
                        throw new Error("FlareSolverr lieferte ungueltiges XML/HTML (Cloudflare Block?).");
                    }
        ...
        const nativeRes = await axios.get(url, { timeout: 12000, headers: { "User-Agent": "Mozilla/5.0" } });
        
        // VALIDIERUNG auch für den nativen Request, um Fake-Success zu verhindern
        if (nativeRes.data && typeof nativeRes.data === "string") {
            if (!nativeRes.data.includes("<rss") && !nativeRes.data.includes("<channel")) {
                throw new Error("Nativer Request wurde von Cloudflare blockiert (HTTP 200 HTML statt RSS).");
            }
        }
        ...
        return scrapeQueue.enqueue(async () => {
            try {
                ...
                while ((rowMatch = rowRegex.exec(html)) !== null) {
                    ...
                    if (sizeMatch) size = sizeMatch[1] + " Bytes"; // Vereinfacht fuer die Logik
