...
// Adjusted Timeouts to prevent Promise Race-Conditions & Max Concurrency.
//===============
...
// Concurrency auf 20 erhoeht, um jegliche Warteschlangen-Staus zu verhindern
...
async function fetchWithBypass(url, domainName) {
    return scrapeQueue.enqueue(async () => {
        if (FLARESOLVERR_URL) {
            try {
                // Timeouts drastisch reduziert (7s), damit der native Fallback sicher feuern kann
        ...
        // Timeout auf 5s gesetzt. Gesamtdauer (FS+Native) ist max 12-14s.
...
async function searchSukebeiForHentai(queryOriginal) {
    ...
    const searchExecutionPromise = (async () => {
        ...
        // Absolutes Limit auf 14 Sekunden angehoben.
